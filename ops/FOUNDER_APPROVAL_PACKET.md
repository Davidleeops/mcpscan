...
That command writes the DNS packet, Stripe setup packet, DNS records CSV, and Stripe products CSV into `ops/generated-launch-packets/` for the default `mcpscan.online` lane. Open `ops/cheap-launch-packet-console.html` to use the generated files from one screen.
...
2. Buy `getmcpscan.com` if standard-priced, or use `mcpattest.dev` if you approve the cleaner brand.
