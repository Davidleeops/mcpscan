...
const sandbox = fs.mkdtempSync(path.join(os.tmpdir(), "mcpscan-route-stage-"));
...
const packetFile = path.join(root, "sales/first-10-route-approval-packet-2026-08-14.md");
...
const batchDir = path.join(outputRoot, "2026-08-14_first-10-route-approvals");
...
for (const account of expectedAccounts) {
  ...
  assert(accountManifest.routeBased === true, `${account} manifest missing routeBased flag`);
}

fs.rmSync(sandbox, { recursive: true, force: true });

console.log("First-10 route staging simulation passed.");
