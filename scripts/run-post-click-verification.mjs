...
if (qaFile) {
  ...
  if (applyReturnPacket || fs.existsSync("ops/founder-approval-status.json")) {
    run("approval status consistency", "npm", ["run", "launch:verify-status", "--", "--file", args.file, "--qa-file", qaFile]);
  } else {
    info("Skipping approval status consistency because the return packet was not applied and no status tracker exists.");
  }
} else if (strict) {
